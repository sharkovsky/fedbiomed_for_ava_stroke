...
from fedbiomed.factory.distributed import reduce_sum
from fedbiomed.factory.op import upfirdn2d
...
def apply_color(img, mat):
    ...
    mat_mul = mat[:, :3, :3].transpose(1, 2).view(batch, 1, 3, 3).mean([2,3], keepdim=True)
    mat_add = mat[:, :3, 3].view(batch, 1, 1, 3).mean([3], keepdim=True)
