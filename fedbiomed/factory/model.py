...
from fedbiomed.factory.op import FusedLeakyReLU, fused_leaky_relu, upfirdn2d, conv2d_gradfix
...
class Generator(nn.Module):
    ...
    def forward(
        ...
        if not input_is_latent:
            ...
            if len(labels) > len(styles[0]):
                assert len(labels) == len(styles), f"{labels.shape}, {len(styles)}, {styles[0].shape}"
...
class Discriminator(nn.Module):
    def __init__(self, size, n_domains, channel_multiplier=2, blur_kernel=[1, 3, 3, 1]):
        super().__init__()
        ...
        self.pixel_norm = PixelNorm()
        self.label_embed = EqualLinear(29+n_domains, channels[4]) # TODO: pass n. of labels
        self.encoding_embed = EqualLinear(768, channels[4]) # TODO: pass n. of labels
    ...
    def forward(self, input, labels):
        ...
        if encodings is not None:
            encodings = self.encoding_embed(encodings) 
            #encodings = .8*encodings + .2*torch.randn_like(encodings)
        ...
        if encodings is not None:
            out *= encodings
        out = torch.sum(out, dim=1, keepdim=True)/np.sqrt(out.shape[1])
        return out
