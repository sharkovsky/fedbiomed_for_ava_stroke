...
def extract_feature_from_samples(
    generator, inception, truncation, truncation_latent, batch_size, n_sample, device, labels, zaxes
):
    ...
    for i, batch in enumerate(tqdm(batch_sizes)):
        ...
        zaxis = torch.tensor(zaxes[i], device=device)
        
        label = torch.nn.functional.one_hot(label, num_classes=generator.n_domains).float()# TODO: generalize n. of labels
        zaxis = torch.nn.functional.one_hot(zaxis, num_classes=29).float()
        label = torch.cat([label, zaxis], dim=1)
        
        img, _ = g([latent], label, truncation=truncation, truncation_latent=truncation_latent)
...
if __name__ == "__main__":
    ...
    features = extract_feature_from_samples(
        g, inception, args.truncation, mean_latent, args.batch, args.n_sample, device, labels, zaxes
    ).numpy()
